...
func TestFunctionObject(t *testing.T) {
	...
	if !ok {
		t.Fatalf("not a function, got %T", evaluated)
	}

	if len(fn.Parameters) != 1 {
		t.Fatal("wrong number of parameters, expected 1")
	}

	if fn.Parameters[0].String() != "x" {
		t.Fatalf("parameter is not x, got %s", fn.Parameters[0])
	}
...
func TestStringLiteral(t *testing.T) {
	...
	if !ok {
		t.Fatalf("object is not String %T", evaluated)
	}

	if str.Value != "foobar" {
		t.Errorf("wrong value, got %s", str.Value)
	}
}
